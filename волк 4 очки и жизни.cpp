...
int main()
{
	...
	Font font;
	font.loadFromFile("font/bahnschrift.ttf");//передаем нашему шрифту файл шрифта
	Text text("", font, 20);

	Font fontstonetime;
	fontstonetime.loadFromFile("font/digifaw.TTF");//передаем нашему шрифту файл шрифта
	Text textstonetime("", fontstonetime, 20);


	Font fontheater;
	fontheater.loadFromFile("font/bahnschrift.ttf");//передаем нашему шрифту файл шрифта
	Text textsheater("", fontheater, 20);
	...
	while (window.isOpen())
	{
		...
		playerBall << p.ball;
		text.setString("ball : " + playerBall.str());
		text.setPosition(view.getCenter().x-320, view.getCenter().y-90); //место текста
		window.draw(text);
		if (p.ball == 0)clockstone.restart();
		if (p.ball > 0 && p.ball<7) {


				float timestone = clockstone.getElapsedTime().asSeconds();
				ostringstream playertimestone;
				playertimestone << timestone;
				textstonetime.setString("time: " + playertimestone.str());
				textstonetime.setPosition(view.getCenter().x, view.getCenter().y); //место текста
				window.draw(textstonetime);
		...
		ostringstream kolheath;
		kolheath << p.health;
		textsheater.setString("heater " + kolheath.str());
		textsheater.setPosition(view.getCenter().x - 300, view.getCenter().y - 100);
		window.draw(textsheater);
