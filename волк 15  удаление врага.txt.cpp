...
int main()
{
	...
	while (window.isOpen())
	{
		...
		//мы в тот лист можем кинуть всех врагов и потои просто проверять их по имени 
		//просто обновляем и рисуем + удаляем
		for (auto it = easyEnemy.begin(); it != easyEnemy.end();)//говорим что проходимся от начала до конца
		{
			...
			//если врага сдог то удоляем его
			if ((*it)->life == false) { it=easyEnemy.erase(it);  }// если этот объект мертв, то удаляем его
		...
		//типо для убийств
		for (auto it = easyEnemy.begin(); it != easyEnemy.end(); it++)//проходимся по эл-там списка
		{
