...
int main()
{
	...
	while (window.isOpen())
	{
		...
		if (p.life) {
			text timeStone("digifaw.TTF", "time : ", timestone, -310, -70);
			if (p.ball == 0)clockStone.restart();
			if (p.ball > 0 && p.ball < 7) {

				timestone = clockStone.getElapsedTime().asSeconds();

				timeStone.sleditCam();
				window.draw(timeStone.txt);
			}
		...
		if (p.life == false) {
			...
			text gameOver("digifaw.TTF", "GameOver ball:  ", p.ball, -300, 0);
			gameOver.txt.setCharacterSize(30);
			gameOver.txt.setFillColor(Color(255, 0, 0));
			window.draw(gameOver.txt);
